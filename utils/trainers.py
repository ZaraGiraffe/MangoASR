...
############################################################################################
    ...
    def concatenate(self, mas: list[torch.Tensor]) -> torch.Tensor:
        ...
        :param mas: list of torch.Tensor such that every dimension > 1 must be equal
        :return: torch.Tensor - padded and concatenated tensor
        """
        device = mas[0].device
        max_length = max([arr.shape[1] for arr in mas])
        padded_mas = []
        for i, arr in enumerate(mas):
            arr_shape = list(arr.shape)
            arr_shape[1] = max_length - arr_shape[1]
            padding_tensor = torch.full(arr_shape, self.model.config.pad_token_id).to(device)
            padded_arr = torch.concatenate((arr, padding_tensor), dim=1)
